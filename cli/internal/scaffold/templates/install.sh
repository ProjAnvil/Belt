...
if [ -d "$SKILL_SRC" ]; then
    for f in "$SKILL_SRC"/*; do
        fname=$(basename "$f")
        [ "$fname" = "reference" ] && continue   # handled separately below
...
# ── reference/ directory ──────────────────────────────────────────────────
# Created as a real directory (not a symlink) so app.json can live alongside
# component reference files without writing back into the project tree.
REFERENCE_DST="$CLAUDE_SKILLS_DIR/$APP_NAME/reference"
mkdir -p "$REFERENCE_DST"

# Copy app.json (not linked — it holds per-installation runtime config)
APP_JSON_SRC="$SCRIPT_DIR/app.json"
if [ -f "$APP_JSON_SRC" ]; then
    cp "$APP_JSON_SRC" "$REFERENCE_DST/app.json"
    echo -e "${GREEN}  copied${NC} reference/app.json"

    # Create the app data directory declared in app.json
    APP_DIR=$(python3 -c "import json,pathlib; d=json.loads(pathlib.Path('$APP_JSON_SRC').read_text()); print(pathlib.Path(d.get('app_dir','')).expanduser())" 2>/dev/null || true)
    if [ -n "$APP_DIR" ]; then
        mkdir -p "$APP_DIR"
        echo -e "${GREEN}  created${NC} app dir: $APP_DIR"
    fi
else
    echo -e "${YELLOW}  (no app.json found — skipping)${NC}"
fi

# Link component reference files/directories (each component lives in its own subdirectory)
if [ -d "$SKILL_SRC/reference" ]; then
    for entry in "$SKILL_SRC/reference"/*; do
        [ -e "$entry" ] || continue
        fname=$(basename "$entry")
        link_or_copy "$entry" "$REFERENCE_DST/$fname" "reference/$fname"
    done
fi
