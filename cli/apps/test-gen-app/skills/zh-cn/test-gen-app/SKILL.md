...
2. [第二步，例如："解析内容并提取 X"]
3. [第三步，例如："携带提取的数据调用 scripts/test-gen-app/run.py"]
...
| 资源 | 路径 | 说明 |
|------|------|------|
| 主脚本 | `scripts/test-gen-app/run.py` | 主处理逻辑 |

调用主脚本：

```bash
python3 {skill_path}/scripts/test-gen-app/run.py [参数]
```

其中 `{skill_path}` 是此 skill 目录的绝对路径（`~/.claude/skills/test-gen-app`）。
...
- `scripts/test-gen-app/` 中的脚本与语言无关，中英文版本共享同一套脚本
